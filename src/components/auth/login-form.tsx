...
import { AlertCircle, Loader2, ArrowRight, Languages, CheckCircle2 } from "lucide-react";
...
export function LoginForm() {
  ...
  // Loading overlay state (shown after successful auth, while the app initializes)
  ...
  const handleSignIn = async () => {
    try {
      ...
      // signIn sets isAuthenticated=true and currentRoute — the page.tsx
      // component will re-render and show AppShell. The overlay stays
      // visible until that happens (the fade-out covers the transition).
    } catch {
  ...
  const handleForgotPassword = async () => {
    if (!forgotEmail.trim()) {
      setForgotError(isAr ? "يرجى إدخال البريد الإلكتروني" : "Please enter your email");
      return;
    }
    ...
    try {
      ...
      setForgotError((e as Error).message || (isAr ? "حدث خطأ" : "An error occurred"));
    } finally {
  ...
  return (
    <>
      {/* ─── Loading Overlay ────────────────────────────────────────────
          Shown after successful auth while the app initializes.
          Logo + spinner + "جاري تسجيل الدخول..." + fade transition */}
      {showLoadingOverlay && (
                ...
                {forgotSuccess ? (
                  <div className="text-center space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground mt-1.5">
                        {isAr
                          ? "إذا كان البريد الإلكتروني مسجلاً، ستصلك رسالة بإعادة تعيين كلمة المرور."
                          : "If the email is registered, you'll receive a password reset link shortly."}
                      </p>
                    </div>
                    <Button variant="outline" className="w-full h-11" onClick={() => { setForgotOpen(false); setForgotSuccess(false); setForgotEmail(""); }}>
                  ...
                  <>
                    ...
                    <div className="space-y-4">
                      ...
                      <Button variant="ghost" className="w-full text-sm" onClick={() => setForgotOpen(false)}>
                        {isAr ? "العودة لتسجيل الدخول" : "Back to Login"}
                      </Button>
                    </div>
                  </>
