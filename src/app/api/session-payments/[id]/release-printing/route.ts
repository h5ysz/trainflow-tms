...
export const POST = withModuleAction("sessions", "edit", async ({ req, params, user }) => {
  ...
  if (contractors.length > 0) {
    await db.notification.createMany({
      data: contractors.map((c) => ({
        ...
        message: `Certificate printing has been released for session ${sp.session.refNumber}. You can now download certificates.`,
        messageAr: `تم السماح بطباعة الشهادات لجلسة ${sp.session.refNumber}. يمكنك الآن تنزيل الشهادات.`,
        type: "SUCCESS",
        category: "FINANCIAL",
        updatedAt: now,
      })),
  ...
  await audit({
    ...
    description: `Certificate printing released for ${sp.company?.name} (Session ${sp.session.refNumber}) — full payment verified`,
    descriptionAr: `تم السماح بطباعة الشهادات لـ ${sp.company?.name} (جلسة ${sp.session.refNumber}) — تم التحقق من الدفع الكامل`,
    req,
