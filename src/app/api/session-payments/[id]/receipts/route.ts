...
export const POST = withModuleAction("sessions", "edit", async ({ req, params, user }) => {
  ...
  const sp = await db.sessionPayment.findUnique({
    where: { id },
    include: { company: { select: { name: true } }, trainingSession: { select: { refNumber: true } } },
  });
  if (!sp || sp.deletedAt) return fail("Session payment not found", 404);
  ...
  if (coordinators.length > 0) {
    await db.notification.createMany({
      data: coordinators.map((c) => ({
        ...
        message: `${sp.company?.name ?? "A company"} uploaded a payment receipt for ${amount} ${sp.currency} (Session ${sp.trainingSession.refNumber}). Please verify.`,
        messageAr: `${sp.company?.name ?? "شركة"} رفعت إيصال دفع بمبلغ ${amount} ${sp.currency} (جلسة ${sp.trainingSession.refNumber}). يرجى التحقق.`,
        type: "INFO",
        category: "FINANCIAL",
        updatedAt: now,
      })),
  ...
  await audit({
    ...
    description: `Payment receipt uploaded: ${amount} ${sp.currency} by ${user.fullName} for session ${sp.trainingSession.refNumber}`,
    descriptionAr: `تم رفع إيصال دفع: ${amount} ${sp.currency} بواسطة ${user.fullName} لجلسة ${sp.trainingSession.refNumber}`,
    req,
