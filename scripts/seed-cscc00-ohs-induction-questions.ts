// GCCLAB TMS — Seed the bilingual question bank for "OHS General Induction" (CSCC00)
// =====================================================================
// Source: OHS General Induction question content from CSCC09 seed, targeting the
// existing CSCC00 course.
// Does NOT create a new course — only finds existing CSCC00 by code.
...
async function findCourse() {
  ...
  if (!course) {
    throw new Error(`Course ${COURSE_CODE} not found. Run the main seed first to create it.`);
  }
  console.log(`Found existing course ${COURSE_CODE} (${course.refNumber}).`);
  return course;
}
...
async function run() {
  ...
  console.log("=== SEED CSCC00 OHS GENERAL INDUCTION QUESTIONS COMPLETE ===");
  console.log(`Course : ${COURSE_CODE} — ${course.title} (${course.refNumber})`);
